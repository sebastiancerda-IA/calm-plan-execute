    ...
    <div className="space-y-6">
      ...
      {agent.metrics.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {agent.metrics.map((m) => (
            <MetricTile key={m.label} label={m.label} value={m.value} trend={m.trend} sparkline={m.sparkline} color={agent.color} />
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-md border border-border bg-card p-3">
          ...
          <p className="text-sm text-foreground font-mono mt-1">{agent.trigger}</p>
        </div>
        <div className="rounded-md border border-border bg-card p-3">
          <span className="text-[10px] text-muted-foreground uppercase">Última ejecución</span>
          <p className="text-sm text-foreground font-mono mt-1">{timeAgo(agent.lastRun)}</p>
        </div>
        <div className="rounded-md border border-border bg-card p-3">
          <span className="text-[10px] text-muted-foreground uppercase">Error Rate</span>
          <p className="text-sm text-foreground font-mono mt-1">{(agent.errorRate * 100).toFixed(1)}%</p>
        </div>
        <div className="rounded-md border border-border bg-card p-3">
          <span className="text-[10px] text-muted-foreground uppercase">Items 24h</span>
          <p className="text-sm text-foreground font-mono mt-1">{agent.itemsProcessed24h}</p>
        </div>
      </div>
              ...
              <tbody>
                {emails.map((email) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-2 text-muted-foreground font-mono whitespace-nowrap">{email.deadline || '—'}</td>
